...
$sqleditcar = "SELECT * from tbl_ourshop WHERE data=$ourshop";
$resultcar=mysqli_query($conn,$sqleditcar);
$user_datacar= mysqli_fetch_assoc($resultcar);

$core = $user_datacar['ourshop'];
$text = $user_datacar['text'];
	...
	<div class=>
		...
		<div class=panel-content>
			...
			<form method=POST action=manageCarousel.php>
                <tr>
                    <td><?php echo "$core"; ?></td>
                    <td><textarea name=text><?php echo "$text"; ?></textarea></td>
                </tr>
                <tr>
					<input type=hidden name=carID value=<?php echo "$ourshop"; ?> >
                </tr>
                <tr>					
                    <td colspan=2><center><input type=submit name=ourshop value=SAVE></center></td>				
                </tr>
